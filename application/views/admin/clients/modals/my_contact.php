...
<div class="modal fade" id="contact" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    echo ' hide';
                        ...
                        <?php } ?>
                        <hr />
                        <?php if (isset($contact) && !empty($contact->signature)): ?>
                            <?php
                                $url = base_url('uploads/register/superadmin/' . $contact->id . '/signature.png');
                                if (IS_TENANT) {
                                    $url = base_url('uploads/register/tenant/' . getSubDomain() . '/' . $contact->id . '/signature.png');
                                }
                            ?>
                            <p class="bold"><?php echo _l('signature') ?></p>
                            <img src="<?php echo $url; ?>" class="mbot10">
                            <p><b><?php echo _l('contract_signed_by') . ': ' . $contact->firstname . ' ' . $contact->lastname ?></b></p>
                            <p><b><?php echo _l('contract_signed_date') . ': ' . _dt($contact->datecreated) ?></b></p>
                            <p><b><?php echo _l('contract_signed_ip') . ': ' . $contact->acceptance_ip ?></b></p>
                            <hr />
                        <?php endif ?>
